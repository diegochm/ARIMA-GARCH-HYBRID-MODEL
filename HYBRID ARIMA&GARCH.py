...
plt.title("Proyección de precios con ARIMA(1,1,1) + GARCH(1,1)")
plt.xlabel("Días futuros")
plt.ylabel("Precio de la plata")
plt.legend()
plt.show()
...
plt.plot(x, forecast_prices, label="Precio proyectado (rolling)")
plt.fill_between(x, lower_band, upper_band, alpha=0.3, label="±2σ (GARCH)")
real_prices = data.loc[index_forecast, "Close"].values 
plt.plot(x, real_prices, label="Precio real", linestyle='--', color='black')
...
def analyse_strategy(returns, rf=0):
    ...
    ax[0].set_ylabel('Crecimiento del Capital')
    ax[0].legend()
    ...
    plt.suptitle("Análisis de la Estrategia")
    plt.tight_layout()
    plt.show()
